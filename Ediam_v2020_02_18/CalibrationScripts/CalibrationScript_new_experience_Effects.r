...
#save calibrated parameters
  calib.params<<-c(
                  ...
                  lrng.re.N = round(out$par[11],4), #Note: no experience accumulation effects considered in this experiment
                  lrng.ce.N = round(out$par[12],4), #Note: no experience accumulation effects considered in this experiment
                  lrng.re.S = round(out$par[13],4), #Note: no experience accumulation effects considered in this experiment
                  lrng.ce.S = round(out$par[14],4), #Note: no experience accumulation effects considered in this experiment
...
#print as a file
  ...
  calOut$seq<-i
  write.csv(calOut,paste0(dir.calib,"Bootstrap_Out_Exp\\","bootsIter_",i,".csv"),row.names=FALSE)
}
